...
public class ProcessFallbackAuthorizationUseCase implements ProcessFallbackAuthorizationInputPort {
    private static final Logger logger = LoggerFactory.getLogger(ProcessFallbackAuthorizationUseCase.class);
    ...
    public ProcessFallbackAuthorizationUseCase(
    ...
    public String process(TransactionPayload transactionPayload) {
        ...
        if (distributedLockManagerOutputPort.tryLock(lockKey, 100)) {
            try {
                ...
                if (!isApproved && category != BalanceCategory.CASH) {
                    logger.info("Insufficient balance in category: {}. Trying CASH category.", category);
                    isApproved = processTransactionForCategory(accountBalance, transactionPayload.getTotalAmount(), BalanceCategory.CASH);
                }
    ...
    private boolean processTransactionForCategory(AccountBalance accountBalance, double totalAmount, BalanceCategory category) {
        boolean result = switch (category) {
            ...
            case CASH -> adjustBalance(accountBalance::getBalanceCash, accountBalance::setBalanceCash, totalAmount);
        };
